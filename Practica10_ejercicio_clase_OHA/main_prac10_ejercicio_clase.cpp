...
//para mover el coche se usan las flechas derecha e izquierda
...
void animate(void)
{
	if (play) {
		if (direction_right) {
			movKit_z += 0.03f; //para mover o detener el carro_ iniciar i detener la animacion
			rot_llanta += 1.1f;
		}
		else {
			movKit_z -= 0.03f; //para mover o detener el carro_ iniciar i detener la animacion
...
void display(Shader shader, Model modelo, Model llantas)//ahora si resiven parametros: shader, modelo y llantas
{
	shader.use();
	...
	shader.setMat4("model", model);
	modelo.Draw(shader);//para dibujar el modelo
...
int main()
{
	...
	// Load models
	Model ourModel =((char *)"Models/kitt.obj");//para cargar el modelo
	Model llantasModel = ((char *)"Models/llanta2/llanta2.obj");//para cargar el modelo de la llanta
